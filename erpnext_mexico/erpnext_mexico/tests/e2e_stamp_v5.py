"""E2E Test v5: Correct postal code 42501 for EKU9003173C9."""
import frappe


def run():
    print("=== E2E Stamp Test v5 (CP=42501) ===\n")

    # Create postal code
    if not frappe.db.exists("MX Postal Code", "42501"):
        frappe.get_doc({"doctype": "MX Postal Code", "code": "42501",
                        "description": "Actopan, Hidalgo"}).insert(ignore_permissions=True)
        frappe.db.commit()
    ...
    cust.mx_nombre_fiscal = "ESCUELA KEMPER URGATE"
    cust.mx_regimen_fiscal = "601"
    cust.mx_domicilio_fiscal_cp = "42501"
    cust.save(ignore_permissions=True)
    frappe.db.commit()
    print(f"Company CP=42501, Customer CP=42501")
